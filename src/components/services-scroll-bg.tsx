...
/**
 * Services cyber background — scroll parallax kept, paint cost reduced
 * (no backdrop-filter, fewer layers, idle when off-screen).
 */
export function ServicesScrollBg({ targetRef }: ServicesScrollBgProps) {
  ...
  if (!isNear) {
    return (
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 services-cyber-sky opacity-60"
      />
    );
  }
  ...
  return (
    <>
      ...
      >
        <div className="services-cyber-sky absolute inset-[-10%]" />
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-gradient-to-b from-background/40 via-background/25 to-background/55 transition-colors duration-500"
      />
      <div
        aria-hidden
